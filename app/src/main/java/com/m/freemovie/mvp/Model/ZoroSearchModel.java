...
public class ZoroSearchModel {
    public static void getZoRoSearchQuery(String search,final Callback<ZoRoSearchBean> callback) {
        NetworkingUtils.getTagalogDub()
                .getSearchZoro(search)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Observer<ZoRoSearchBean>() {
                    @Override
                    public void onSubscribe(Disposable d) {}

                    @Override
                    public void onNext(ZoRoSearchBean data) {
