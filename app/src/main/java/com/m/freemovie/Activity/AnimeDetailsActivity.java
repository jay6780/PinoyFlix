...
public class AnimeDetailsActivity extends AppCompatActivity implements AnimeDetailsContract.View {
    ...
    @Override
    public void getMiRuRoDetail(MiRuRoDetailBean miRuRoDetailBean) {
        if (binding == null) return;
        if (miRuRoDetailBean != null) {
            Set<String> seenEpisodes = new HashSet<>();
            this.imageUrl = miRuRoDetailBean.getCover();
            this.animeTitle = miRuRoDetailBean.getTitle();
            this.overView = miRuRoDetailBean.getSynopsis();
            if (miRuRoDetailBean.getTotalEpisodes() != 0) {
                String episode = String.valueOf(miRuRoDetailBean.getTotalEpisodes());
                if (!seenEpisodes.contains(episode)) {
                    seenEpisodes.add(episode);
                }
            }
            animeDetailsBeanList.add(new AnimeDetailsBean(url, imageUrl, animeTitle, seenEpisodes.size(), ""));
            animeSeasonAdapter.setNewData(animeDetailsBeanList);
            binding.tvDescription.setText(overView);
            binding.tvOriginal.setText(animeTitle);
            binding.tvTitle.setText(animeTitle);
