...
//        Log.d("PlayerDomain", "val: " + player);
    ...
    private void rotateScreen() {
        ...
        binding.playerView.setLayoutParams(new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, RelativeLayout.LayoutParams.MATCH_PARENT));
    ...
    private void portraitFull() {
        ...
        binding.playerView.setLayoutParams(playerParams);
        if (binding.playerView != null) {
            binding.playerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_ZOOM);
        }
        ...
        if (subtitleView != null) {
            subtitleView.setFixedTextSize(TypedValue.COMPLEX_UNIT_SP, 15f);
        }
        new WindowUtils(this, true, false);
    }
...
//                Log.d("ScraperDomain", "Page finished on: " + url + " (domain: " + player + ")");
                        ...
                        runOnUiThread(() -> {
                            if (!hasStartedPlayback) {
                                ...
                                String msg = (statusCode == 404)
                                        ...
                                        : "Server error";
                                Toast.makeText(OtherWebviewActivity.this, msg, Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                }
                        ...
                        if (errCode == ERROR_HOST_LOOKUP || errCode == ERROR_CONNECT || errCode == ERROR_TIMEOUT) {
                            runOnUiThread(() -> {
                                if (!hasStartedPlayback) {
                                    ...
                                    Toast.makeText(OtherWebviewActivity.this, "Failed to connect to server. Please select another server.", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    }
        ...
        scraperTimeoutRunnable = () -> {
            if (!hasStartedPlayback && pendingStreamUrl == null) {
                ...
                binding.playerView.setVisibility(View.VISIBLE);
                Toast.makeText(OtherWebviewActivity.this, "Server response slow. Please select another server.", Toast.LENGTH_SHORT).show();
            }
        };
...
//                Log.w("VideoStatus", "URI status check exception: " + e.getMessage());
                if (e instanceof java.io.FileNotFoundException || (e.getMessage() != null && e.getMessage().contains("404"))) {
                    runOnUiThread(() -> {
                        ...
                        Toast.makeText(OtherWebviewActivity.this, "Video not found (404). Please select another server.", Toast.LENGTH_SHORT).show();
                    });
                    hasStartedPlayback = false;
                    pendingStreamUrl = null;
                    return;
    ...
    private void showSubtitleDialog() {
        ...
        if (displayNames.size() == 1) { // Only "Off" is in the list
            new AlertDialog.Builder(this, R.style.AlertDialogTheme)
                    .setTitle("Subtitles")
                    .setMessage("No subtitle tracks found in this video stream.\n\nSubtitles depend on the video server. If you need subtitles, please try selecting another server below.")
